...
for (i in seq_len(nrow(td))) {
    ...
    numParent <- rbind(numParent, tapply(clusters %in% parentClusters,
                                         samples, sum))
    numNode <- rbind(numNode, tapply(clusters %in% childClusters, samples,
                                     sum))
}
...
glmm_p_vals <- data.frame(matrix(nrow=nrow(numNode), ncol=4))
colnames(glmm_p_vals) <- c("glmm_coef_total", "glmm_p_val_total",
                           "glmm_coef_parent", "glmm_p_val_parent")

n_cells_smp <- colSums(numNode[tree_sig_df %>% filter(isTip) %>% pull(node),])

experiment_info <- metadata(sce)$experiment_info
contrast <- t(createContrast(c(0, 1)))
...
for (i in seq_len(nrow(numNode))) {
    ...
    tryCatch({
        ...
        # note: divide by total number of cells per sample (after filtering) to get
        # proportions instead of counts
        y <- numNode[i, ] / numParent[i,]
        data_i <- cbind(y, n_cells_smp, formula$data)
        # fit model
        # note: provide proportions (y) together with weights for total number of cells per
        # sample (n_cells_smp); this is equivalent to providing counts
        fit <- glmer(formula$formula, data = data_i, family = "binomial", weights = numParent[i,])
        # test contrast
        test <- multcomp::glht(fit, contrast)
        # return p-value
...
y <- DGEList(counts=numNode, group = cl[colnames(numNode)], lib.size = rep(1,ncol(numNode)))
design <- model.matrix(~cl[colnames(numNode)])
# I have logged it because of getOffset()
y$offset <- log(numParent+1)
...
## EdgeR total
y <- DGEList(counts=numNode, group = cl[colnames(numNode)])
y <- estimateDisp(y)
fit <- glmFit(y)
lrt <- glmLRT(fit,coef=2)
top <- topTags(lrt, n = 10000)
